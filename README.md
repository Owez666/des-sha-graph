...
### 4. Работа Singleton (База данных)

В проекте используется паттерн **Singleton** для подключения к базе данных **SQLite**.  
Это позволяет создать только один экземпляр соединения с базой данных и избежать повторного открытия файла при каждом запросе от сервера.
При запуске сервер обращается к единственному экземпляру БД, который обеспечивает обработку всех запросов (регистрация, авторизация и т.д.).

Пример работы сервера:

<img width="500" height="300" alt="image" src="https://github.com/user-attachments/assets/699f7bea-70cf-4daa-b071-4d59e3f8c577" />

### 5. Проверка содержимого базы
