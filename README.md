...
На скриншоте показано, как сервер обрабатывает **два подключения одновременно**  
(два клиента, запущенные через **PuTTY**, подключены к одному серверу на порту **33333**).
Первый клиент выполняет регистрацию, второй клиент подключается параллельно и входит под тем же логином. Оба подключения обслуживаются **одновременно**, что подтверждает корректную работу сервера с несколькими клиентами через **QTcpServer** и **QTcpSocket**.

<img width="700" height="500" alt="image" src="https://github.com/user-attachments/assets/c260e5f1-6474-4bb5-9c87-bcefbc085c10" />

### 6. Проверка содержимого базы
...
- Ошибки при неверном вводе корректно обрабатываются. (При повторной регистрации с тем же логином сервер корректно сообщает об ошибке) 
- Сервер успешно принимает несколько одновременных подключений.
